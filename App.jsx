...
function Auth() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070d1a", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 360 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontSize: 32, marginBottom: 8 }}>⛳</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: "#facc15", fontFamily: "'Georgia',serif" }}>Caddie Ledger</div>
          <div style={{ fontSize: 10, color: "#334155", letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 4 }}>Every loop · every dollar · every day</div>
        </div>
        <div style={{ background: "#0a1425", border: "1px solid #1a2744", borderRadius: 10, padding: 24 }}>
          <div style={{ display: "flex", marginBottom: 20, borderBottom: "1px solid #1a2744" }}>
            {["login","signup"].map(m => (
              <button key={m} onClick={() => { setMode(m); setMsg(""); }} style={{
                flex: 1, padding: "10px", background: "none", border: "none",
                borderBottom: mode === m ? "2px solid #facc15" : "2px solid transparent",
                color: mode === m ? "#facc15" : "#475569",
          ...
          <div style={S.miniLabel}>Email</div>
          <input type="email" value={email} onChange={e => setEmail(e.target.value)}
            placeholder="you@email.com" style={{ ...S.input, marginBottom: 14 }} />
          <div style={S.miniLabel}>Password</div>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)}
            placeholder="password" style={{ ...S.input, marginBottom: 20 }} />
          {msg && <div style={{ fontSize: 12, color: msg.startsWith("Check") ? "#22c55e" : "#f87171", marginBottom: 14, textAlign: "center" }}>{msg}</div>}
          <button onClick={handle} disabled={loading} style={{ ...S.primaryBtn, opacity: loading ? 0.6 : 1 }}>
...
function LogDay({ days, setDays, roster, userId }) {
  ...
  function addBag(li) {
    setLoops(prev => prev.map((lp, i) => i !== li ? lp : { ...lp, bags: [...lp.bags, blankBag()] }));
  }
  function removeBag(li, bi) {
    setLoops(prev => prev.map((lp, i) => i !== li ? lp : { ...lp, bags: lp.bags.filter((_, j) => j !== bi) }));
  }

  async function save() {
    const activeLoops = loops.slice(0, count);
    const hasData = activeLoops.some(lp => lp.bags.some(b => b.name.trim() || Number(b.fee) || Number(b.tip)));
    if (!hasData) return;
    setSaving(true);
    const savedLoops = activeLoops.map(lp => ({
      ...
      bags: lp.bags
        ...
        .map(b => ({ ...b, name: b.name.trim(), fee: Number(b.fee)||0, tip: Number(b.tip)||0, total: (Number(b.fee)||0)+(Number(b.tip)||0) })),
    })).filter(lp => lp.bags.length > 0);
    const entry = { id: uid(), date, loops: savedLoops, user_id: userId };
    const { error } = await supabase.from("days").insert(entry);
    if (!error) {
  ...
  return (
    <div style={S.card}>
      ...
      {loops.slice(0, count).map((lp, li) => (
        <LoopBlock key={lp.id} loop={lp} loopIndex={li} loopCount={count}
          onChange={updBag} onChangeType={v => updType(li, v)}
          onAddBag={addBag} onRemoveBag={removeBag} roster={roster} />
      ))}
      {dayTotal > 0 && (
      ...
      <button onClick={save} disabled={saving} style={{
        ...
        {saving ? "Saving..." : flash ? "Saved!" : "Save Day"}
      </button>
    </div>
  );
}
...
function GolferRoster({ roster }) {
  ...
  return (
    <div style={S.card}>
      ...
      {roster.length === 0 ? (
        <p style={{ color: "#475569", textAlign: "center", marginTop: 48, fontSize: 14 }}>No golfers logged yet.</p>
      ) : (
        <>
          <input type="text" placeholder="Search golfers..." value={search}
            onChange={e => setSearch(e.target.value)} style={{ ...S.input, marginBottom: 16 }} />
...
function Expenses({ expenses, setExpenses, userId }) {
  ...
  async function savePending() {
    ...
    setSaving(true);
    const entries = pending.map(p => ({ ...p, date: expDate, id: uid(), user_id: userId }));
  ...
  return (
    <div style={S.card}>
      ...
      <input type="date" value={expDate} onChange={e => setExpDate(e.target.value)} style={{ ...S.input, marginBottom: 20 }} />
        ...
        <>
          ...
          {pending.map(p => (
            <div key={p.id} style={S.pendingRow}>
              ...
              <button onClick={() => removePending(p.id)} style={S.xBtn}>X</button>
            </div>
          ))}
          <div style={S.totalPreview}>
          ...
          <button onClick={savePending} disabled={saving} style={{
            ...S.primaryBtn,
            background: flash ? "#7f1d1d" : "#ef4444",
            color: flash ? "#fecaca" : "#fff",
            opacity: saving ? 0.6 : 1,
          }}>
            {saving ? "Saving..." : flash ? "Saved!" : `Save ${pending.length} Expense${pending.length > 1 ? "s" : ""}`}
          </button>
        </>
      )}
    </div>
              ...
              return (
                <div key={day.id} style={{ ...S.histCard, borderLeft: "3px solid #22c55e" }}>
                  <div style={S.histHead}>
                    ...
                    <button onClick={() => delDay(day.id)} style={S.xBtn}>X</button>
                  </div>
                  {(day.loops || []).map((lp, li) => {
                      ...
                      <div key={lp.id} style={{ marginTop: 8, paddingLeft: 8, borderLeft: `2px solid ${lt.color}` }}>
                        ...
                        {lp.bags.map((b, bi) => (
                          <div key={b.id} style={S.golferRow}>
                            <span style={{ fontSize: 9, color: bi === 0 ? "#64748b" : "#f59e0b", fontWeight: 800, letterSpacing: "0.08em", width: 42, flexShrink: 0 }}>BAG {bi + 1}</span>
            ...
            {g.exps.map(exp => (
              <div key={exp.id} style={{ ...S.histCard, borderLeft: "3px solid #ef4444" }}>
                <div style={S.histHead}>
                  ...
                  <button onClick={() => delExp(exp.id)} style={S.xBtn}>X</button>
                </div>
              </div>
            ))}
          </div>
...
function Summary({ days, expenses }) {
  const st = useMemo(() => {
    ...
    const bags2 = bags.length;
    const daysW = days.length;
    const avgLoop = loops ? earned / loops : 0;
    const doubles = days.reduce((s, d) => s + (d.loops || []).filter(lp => lp.bags.length >= 2).length, 0);
    const typeStats = {};
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
            {LOOP_TYPES.map(lt => {
              ...
              return <SplitBox key={lt.value} label={lt.label} color={lt.color} loops={ts.loops} bags={ts.bags} total={ts.total} avg={ts.loops ? ts.total / ts.loops : 0} />;
            })}
          </div>
          {st.cats.length > 0 && (
            <>
                ...
                <div key={c.label} style={S.catRow}>
                  ...
                  <div style={{ width: 90, marginRight: 10 }}>
                    <div style={{ height: 3, borderRadius: 2, background: "#1a2744" }}>
                      <div style={{ height: "100%", borderRadius: 2, background: "#ef4444", width: `${st.totalExp > 0 ? Math.round((c.amount / st.totalExp) * 100) : 0}%` }} />
                    </div>
                  </div>
                  <span style={{ color: "#f87171", fontWeight: 800, fontSize: 13, minWidth: 60, textAlign: "right" }}>{fmt(c.amount)}</span>
                </div>
              ))}
...
export default function CaddieLedger() {
  ...
  useEffect(() => {
    if (!session) return;
    supabase.from("days").select("*").eq("user_id", session.user.id).order("date", { ascending: false })
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#070d1a", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#facc15", fontSize: 14, fontFamily: "'Courier New',monospace" }}>Loading...</div>
    </div>
  );
  ...
  return (
    <div style={S.root}>
        ...
        <div style={S.headerInner}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
            ...
            <button onClick={() => supabase.auth.signOut()} style={{
              background: "none", border: "1px solid #1a2744", borderRadius: 4,
              color: "#475569", fontSize: 9, cursor: "pointer", padding: "3px 8px",
      ...
      <main style={S.main}>
        {tab === "log" && <LogDay days={days} setDays={setDays} roster={roster} userId={session.user.id} />}
        {tab === "roster" && <GolferRoster roster={roster} />}
        {tab === "expenses" && <Expenses expenses={expenses} setExpenses={setExpenses} userId={session.user.id} />}
